...
void AutonomousAgentsPath2DEditor::_notification(int p_what) {
	switch (p_what) {
...
void AutonomousAgentsPath2DEditor::_node_removed(Node *p_node) {
	if (p_node == node) {
		node = nullptr;
		hide();
	}
...
bool AutonomousAgentsPath2DEditor::forward_gui_input(const Ref<InputEvent> &p_event) {
	if (!node) {
		return false;
	}
...
void AutonomousAgentsPath2DEditor::_node_visibility_changed() {
	if (!node) {
		return;
	}
...
void AutonomousAgentsPath2DEditor::edit(Node *p_path2d) {
	...
	if (p_path2d) {
		...
		if (node && node->is_connected("visibility_changed", callable_mp(this, &AutonomousAgentsPath2DEditor::_node_visibility_changed))) {
			node->disconnect("visibility_changed", callable_mp(this, &AutonomousAgentsPath2DEditor::_node_visibility_changed));
		}
		node = nullptr;
	}
}

void AutonomousAgentsPath2DEditor::_bind_methods() {
	//ClassDB::bind_method(D_METHOD("_menu_option"),&AutonomousAgentsPath2DEditor::_menu_option);
}

void AutonomousAgentsPath2DEditor::_mode_selected(int p_mode) {
...
void AutonomousAgentsPath2DEditor::_handle_option_pressed(int p_option) {
	PopupMenu *pm;
...
AutonomousAgentsPath2DEditor::AutonomousAgentsPath2DEditor() {
	...
	curve_edit->connect("pressed", callable_mp(this, &AutonomousAgentsPath2DEditor::_mode_selected).bind(MODE_EDIT));
	...
	curve_edit_curve->connect("pressed", callable_mp(this, &AutonomousAgentsPath2DEditor::_mode_selected).bind(MODE_EDIT_CURVE));
	...
	curve_create->connect("pressed", callable_mp(this, &AutonomousAgentsPath2DEditor::_mode_selected).bind(MODE_CREATE));
	...
	curve_del->connect("pressed", callable_mp(this, &AutonomousAgentsPath2DEditor::_mode_selected).bind(MODE_DELETE));
	...
	curve_close->connect("pressed", callable_mp(this, &AutonomousAgentsPath2DEditor::_mode_selected).bind(ACTION_CLOSE));
	...
	menu->connect("id_pressed", callable_mp(this, &AutonomousAgentsPath2DEditor::_handle_option_pressed));
...
void AutonomousAgentsPath2DEditorPlugin::edit(Object *p_object) {
...
AutonomousAgentsPath2DEditorPlugin::AutonomousAgentsPath2DEditorPlugin() {
	path2d_editor = memnew(AutonomousAgentsPath2DEditor);
